...
const tabs: { id: ProfileTab | "today"; label: string; href?: string }[] = [
  { id: "today", label: "Дневник", href: "/today" },
  ...
  return (
    <div className="flex gap-1.5 overflow-x-auto scrollbar-none pb-1">
      {tabs.map(({ id, label, href }) => {
        const active = id === "today" ? false : activeTab === id;
        const link = href ?? `${base}?tab=${id}`;
        return (
          <Link
            key={id}
            href={link}
