...
const LEVEL_NAMES: Record<number, string> = {
  1: "Новичок", 3: "На разгоне", 5: "В ритме", 10: "Заводила", 15: "Движущая сила",
  20: "Мотор района", 25: "Легенда двора", 30: "Сенсей движа", 40: "Икона ритма", 50: "ДВЖ-мастер",
};
...
export const LEVEL_REWARDS: Record<number, string> = (() => {
  const texts = [
    ...
    "Кастомизация маскота",
    "Золотая рамка",
    "Право создавать челленджи",
    "Стикеры в комменты",
    "Титул в ленте",
  ];
  const out: Record<number, string> = {};
  for (let l = 5; l <= 60; l += 5) {
    out[l] = texts[Math.floor((l / 5 - 1) % texts.length)] ?? "Награда уровня";
  }
  return out;
})();
